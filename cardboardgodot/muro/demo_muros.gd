...
## a la pose del muro entrante para ver el encaje. Al llegar al plano el muro se
## pinta verde (paso) o rojo (fallo) segun el chequeo v1.
...
# Pose neutra (parado) por si nadie "adopta" nada: nariz, hombros, codos,
# munecas, caderas, rodillas, tobillos.
const NEUTRA := [
	Vector2(0.0, 1.6),
...
func _refrescar_label(nombre: String) -> void:
	_label.text = "Muro: %s\nPaso: %d   Fallo: %d\nFlechas: mover   ESPACIO: adoptar pose" % [nombre, _ok, _fail]
...
func _spawn() -> void:
	...
	add_child(m)
	_refrescar_label(_pose_entrante["nombre"])
...
func _on_alcanzo_plano(m: Muro) -> void:
	...
	if res["paso"]:
		...
		_fail += 1
	_refrescar_label(m.pose["nombre"])
